    ...
    <div class="container mx-auto px-6">
        <div class="flex justify-center">
            <div class="w-full px-56 flex flex-row">
                <input placeholder="Cari Keyword Disini" type="text" name="keyword" id="keyword" onkeyup="searchProduct()" class="mt-1 focus:ring-brown-500 focus:border-brown-500 block w-3/4 shadow-sm sm:text-sm border-gray-300 rounded-md">
        ...
        <div class="mt-16">
            ...
            <div class="grid gap-y-6 gap-x-2 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 mt-6">
                ...
                <div class="mx-auto rounded-md shadow-md overflow-hidden product-card" id="p-{{ $product->id_produk }}">
                    <div class="flex items-end justify-end h-56 w-56 bg-cover bg-center bg-no-repeat"
                        style="background-image: url('{{ asset('assets/images/' . $product->gambar) }}')">
                        <a  href="{{ route('cart.addToCart') }}?product_id={{ $product['id_produk'] }}&qty=1"
                    ...
                    <div class="flex flex-col px-5 py-3 bg-white">
                        <h3 class="text-gray-700 font-bold uppercase">{{ $product->nama_produk }}</h3>
                        <span class="text-gray-500 mt-2">{{ $product->kuantitas }}</span>
                        <span class="text-gray-500 mt-2">Rp {{ number_format($product->harga_jual, 0, ',', '.') }}</span>
                    </div>
                </div>
                @endforeach
...
<script>
    function searchProduct() {
        const listProduct = document.querySelectorAll('.product-card');

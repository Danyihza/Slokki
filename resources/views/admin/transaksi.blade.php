        ...
        <div class="flex flex-row">
            ...
            <select id="id_transaksi" name="id_transaksi" onchange="getTransaction(this.value)"
                ...
                @foreach ($transactions as $item)
                    <option value="{{ $item->id_transaksi }}">{{ $item->id_transaksi }} - {{ date('Y-m-d', strtotime($item->tanggal_transaksi)) }}</option>
                @endforeach
            </select>
        </div>
    ...
    async function getTransaction(id){
        ...
        html += `
                ...
                <div class="flex flex-row mt-2 items-center">
                    <input name="" type="checkbox" ${data.status.dikemas == 1 ? 'checked' : ''}
                        class="focus:ring-brown-500 h-4 w-4 text-brown-600 border-gray-300 rounded">
                    <span class="ml-2 py-auto">Dikemas</span>
                </div>
                <div class="flex flex-row mt-2 items-center">
                    <input name="" type="checkbox" ${data.status.dikirim == 1 ? 'checked' : ''}
                        class="focus:ring-brown-500 h-4 w-4 text-brown-600 border-gray-300 rounded">
                    <span class="ml-2 py-auto">Dikirim</span>
                </div>
                <div class="flex flex-row mt-2 items-center">
                    <input name="" type="checkbox" ${data.status.diterima == 1 ? 'checked' : ''}
                        class="focus:ring-brown-500 h-4 w-4 text-brown-600 border-gray-300 rounded">
